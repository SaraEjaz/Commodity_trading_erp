...
const normalizeCommissionDeal = (d: any) => {
  if (!d || typeof d !== 'object') return d;
  return {
    ...d,
    total_quantity_mt: toNumber(d.total_quantity_mt),
    quantity_lifted_mt: toNumber(d.quantity_lifted_mt),
    quantity_remaining_mt: toNumber(d.quantity_remaining_mt),
    total_buyer_commission: toNumber(d.total_buyer_commission),
    total_seller_commission: toNumber(d.total_seller_commission),
    commission_received: toNumber(d.commission_received),
  };
};

const normalizeResponse = (data: any) => {
  if (Array.isArray(data)) return data.map(normalizeCommissionDeal);
  if (data && typeof data === 'object' && Array.isArray(data.results)) {
    return { ...data, results: data.results.map(normalizeCommissionDeal) };
  }
  if (data && typeof data === 'object') return normalizeCommissionDeal(data);
  return data;
};
...
export const getCommissionRegister = async (params?: URLSearchParams) => {
  ...
  return items.map((dispatch: any) => ({
    ...
    seller_code: dispatch.invoice_party_code || dispatch.receiver_party_code || dispatch.payment_party,
