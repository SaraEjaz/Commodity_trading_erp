...
import { ordersAPI } from '@/lib/api/orders'; // ✅ Now works with named export
...
export default function OrdersPage() {
  ...
  const { data: orders = [], isLoading, refetch } = useQuery({
    ...
    // ✅ ordersAPI.list() now exists and unwraps paginated response
  ...
  return (
    <div className="space-y-6">
      ...
      <OrdersTable
        orders={orders}
        isLoading={isLoading}
        onEdit={(order) => { setSelectedOrder(order); setIsModalOpen(true); }}
      />

      <OrderModal
        isOpen={isModalOpen}
        onClose={() => { setIsModalOpen(false); setSelectedOrder(null); refetch(); }}
        order={selectedOrder}
      />
    </div>
  );
